import { useMemo } from 'react'
import { Link } from 'react-router-dom'
import { PageHeader } from '../components/common/SectionTitle'
import { getProjectBySlug } from '../data/projects'

export default function MicroLoopPage() {
  const project = useMemo(() => getProjectBySlug('microloop'), [])

  if (!project) {
    return <div>Project not found</div>
  }

  return (
    <div className="min-h-screen py-12 px-8 sm:px-12 lg:px-16">
      <div className="max-w-5xl mx-auto">
        <PageHeader
          title={project.name}
          titleHref={project.links.github}
          backTo="/"
          backLabel="Home"
        />

        <div className="my-10" aria-hidden="true" />

        {/* Background */}
        <section className="mb-0">
          <h2 className="text-3xl font-bold text-surface mb-6">
            Background
          </h2>
          <div className="text-surface space-y-5 text-base leading-relaxed">
            <p>
              In my live setup, I wanted a looper that can sample live audio, but also quantize the capture and playback to an external MIDI clock, like from a drum machine. Put more simply, I wanted to loop audio and have it sound good without worrying about timing. With quantization being the key feature, I couldn't find it in a desktop looper that was under $400, so I decided to build my own using the <a href="https://www.pjrc.com/store/teensy41.html" target="_blank" rel="noopener noreferrer" className="text-accent hover-slant">Teensy 4.1</a> microcontroller. It offers plenty of audio support, a great community, and a powerful Cortex-M7 core so I can cram plenty more features in the future. This was partly inspired by the Elektron Octratrack&apos;s live looping, but I wanted something more focused, immediate, and with minimal menu diving. Loop lengths and repeats are intentionally kept very short for a more glitchy, rhythmic, and robotic vibe based on the music I enjoy playing. All in all, it came out to just $120 for the breadboard prototype!
            </p>
          </div>
        </section>

        <div className="my-10" aria-hidden="true" />

        {/* Demo */}
        <section className="mb-0">
          <h2 className="text-3xl font-bold text-surface mb-6">
            Demo
          </h2>
          <div className="aspect-video w-full">
            <iframe
              className="w-full h-full rounded-lg"
              src="https://www.youtube.com/embed/drOhE85E4RI"
              title="MicroLoop Demo"
              allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
              allowFullScreen
            />
          </div>
        </section>

        <div className="my-10" aria-hidden="true" />

        {/* How to Play */}
        <section className="mb-0">
          <h2 className="text-3xl font-bold text-surface mb-6">
            How to Play
          </h2>
          <div className="text-surface space-y-6 text-base leading-relaxed">
            <img
              src="/assets/images/microloop1.png"
              alt="MicroLoop layout diagram"
              className="w-3/5 mx-auto rounded-lg"
            />
            <p>
              The idea is that you play audio into the device, record up to four different loops, and play them back over the original audio with the STUTTER effect. You have a "scratch work space", where you test out different loops, and commit them to one of four preset slots when you like what you hear. Alongside this, you have a FREEZE effect that sustains the last 30ms of audio, and a CHOKE effect that acts as an instant mute. Each effect has it's own settings menu, where you can toggle different capture/playback parameters to either be quantized to the MIDI clock, or activated freehand. There is also a Global Quantization menu, where you choose between 8 different beat subdivisions for the quantization to snap to.
            </p>

            <div>
              <h3 className="text-xl font-semibold mb-4 underline">Inputs</h3>
              <ul className="space-y-3 list-disc pl-6">
                <li><strong>STUTTER</strong> - Square activates STUTTER, circle is for setting navigation and selection</li>
                <li><strong>FREEZE</strong> - Square activates FREEZE, circle is for setting navigation and selection</li>
                <li><strong>CHOKE</strong> - Square activates CHOKE, circle is for setting navigation and selection</li>
                <li>
                  <strong>FUNC</strong>
                  <ul className="list-disc pl-6 mt-2 space-y-2">
                    <li>FUNC + STUTTER starts loop capture, capture ends automatically via quantization or when FUNC is let go in freehand mode. If STUTTER is still held after loop capture end, the loop is automatically launched right after loop capture.</li>
                    <li>FUNC + PX saves current loop in scratch space to that preset slot, only if the slot is empty. If it is full, that slot's preset is deleted.</li>
                  </ul>
                </li>
                <li><strong>GLOBAL</strong> - Global settings, with quantization currently being the only one. Select between 8 different beat subdivisions, that all settings set to quantized will snap to.</li>
                <li><strong>PX</strong> - Preset buttons, where pressing a full preset button will load that preset to the scratch space. Note that creating a new loop while a preset is selected will NOT overwrite the selected preset, it will just de-select it.</li>
                <li><strong>MIDI DIN IN</strong> - MIDI cable carrying clock signal from any hardware that includes it.</li>
                <li><strong>3.5mm LINE IN</strong> - Stereo audio input, where the device will simply pass through the audio unless an effect is activated.</li>
                <li><strong>MicroUSB Power</strong> - Powers the device, and stays on as long as the cable is plugged in. Later iterations will include a battery with a switch instead.</li>
                <li><strong>MicroSD Card</strong> - For storing the presets. Note that the card must be inserted into the Teensy slot, NOT the Audio Adapter board slot. As only 4 slots are supported for now, any card above 4GB will work with plenty of room to spare.</li>
              </ul>
            </div>

            <div>
              <h3 className="text-xl font-semibold mb-4 underline">Menu Settings</h3>
              <p className="mb-4">
                All setting selections are either Freehand or Quantized, with the exception of GLOBAL's Quantization, which includes 1/32, 1/32T, 1/16, 1/16T, 1/8, 1/8T, 1/4, and 1/4T.
              </p>
              <ul className="space-y-3 list-disc pl-6">
                <li>
                  <strong>STUTTER</strong>
                  <ul className="list-disc pl-6 mt-1">
                    <li>Capture Start</li>
                    <li>Capture End</li>
                    <li>Onset</li>
                    <li>Length</li>
                  </ul>
                </li>
                <li>
                  <strong>FREEZE</strong>
                  <ul className="list-disc pl-6 mt-1">
                    <li>Onset</li>
                    <li>Length</li>
                  </ul>
                </li>
                <li>
                  <strong>CHOKE</strong>
                  <ul className="list-disc pl-6 mt-1">
                    <li>Onset</li>
                    <li>Length</li>
                  </ul>
                </li>
                <li>
                  <strong>GLOBAL</strong>
                  <ul className="list-disc pl-6 mt-1">
                    <li>Quantization</li>
                  </ul>
                </li>
              </ul>
            </div>
          </div>
        </section>

        <div className="my-10" aria-hidden="true" />

        {/* Design & Architecture */}
        <section className="mb-0">
          <h2 className="text-3xl font-bold text-surface mb-6">
            Design &amp; Architecture
          </h2>
          <div className="text-surface space-y-5 text-base leading-relaxed">
            <p>
              This was my first time facing product design at this scale across software, hardware, electrical, and even mechanical engineers, but it has been one of my most rewarding experiences. I used the Teensy line&apos;s own <a href="https://www.pjrc.com/store/teensy3_audio.html" target="_blank" rel="noopener noreferrer" className="text-accent hover-slant">Audio Adapter board</a> with the SGTL5000 codec over I2S, which gave me 16-bit, 44.1 kHz audio to work with over stereo line I/O. More importantly, this allowed me to use the <a href="https://www.pjrc.com/teensy/td_libs_Audio.html" target="_blank" rel="noopener noreferrer" className="text-accent hover-slant">Teensy Audio library</a>, which abstracted a lot of the register-level interactions with the DAC, like buffering the audio with DMA. I experimented with making my own driver from scratch but that is a challenge for another day.
            </p>
            <p>
              One of the biggest challenges was designing the system to be totally deterministic with proper state machines, and having an interface that is responsive enough to use in a live performance. I went with a multithreaded design, giving each element like the MIDI, buttons, encoders, etc. their own processing space and time blocks, ensuring critical threads like audio never block to avoid crackles and glitches. Anything I used for DSP, especially the main loop buffers, had to be allocated beforehand, as dynamic allocation would make the performance very sluggish and at risk of memory leaks. For the thread concurrency, I used the <a href="https://github.com/ftrias/TeensyThreads" target="_blank" rel="noopener noreferrer" className="text-accent hover-slant">Teensy Threads library</a> instead of a full RTOS. Other than being natively compatible with the the Teensy, it excels at providing very lightweight preemptive multithreading without the extra overhead of a full RTOS like heavy synchronization primitives. Other housekeeping included using CMake and Ninja instead of the Arduino IDE, which makes for more reproducible and cross-compatible builds.
            </p>

            <img
              src="/assets/images/microloop2.svg"
              alt="MicroLoop design diagram"
              className="w-[92%] mx-auto rounded-lg"
            />

            <img
              src="/assets/images/microloop3.svg"
              alt="MicroLoop architecture diagram"
              className="w-[68%] mx-auto rounded-lg"
            />
          </div>
        </section>

        <div className="my-10" aria-hidden="true" />

        {/* Future Work */}
        <section className="mb-0">
          <h2 className="text-3xl font-bold text-surface mb-6">
            Future Work
          </h2>
          <div className="text-surface space-y-5 text-base leading-relaxed">
            <p>
              I&apos;ve been working on developing this past a prototype, with additional hardware quality of life features and real users in mind. I have also been getting my feet wet with some real electrical engineering by designing actual PCB&apos;s for the device! I&apos;ve been trying to move away from pre-made modules as much as possible and design most of the circuits from scratch. On the software side, there is a little bit of inconsistency and jitter between the quantization grid measured in MIDI ticks, and the actual audio samples of the incoming audio. This is expected, but I feel like I can handle it in a more stringent way, as I can sometimes hear the quantization is off by just a hair. I&apos;m also overhauling the menu system to be even more immediate, and prioritizing the settings I use the most.
            </p>
            <p>
              As far as actual features go, I think a dedicated pumping &quot;sidechain&quot; effect and some overdrive would be a perfect addition for the feel of the device, as well as a way to scrub the playhead of the loop while it&apos;s in the process of being played. I&apos;m also working on an overdub mode, easier &quot;hotswitching&quot; between presets (and more of them available via banks), and being able to record the loop playback itself to presets as well.
            </p>
          </div>
        </section>

        <div className="my-10" aria-hidden="true" />

        {/* Hardware */}
        <section className="mb-0">
          <h2 className="text-3xl font-bold text-surface mb-6">
            Hardware
          </h2>
          <ul className="text-surface space-y-3 text-base leading-relaxed list-disc pl-6">
            <li><strong>Teensy 4.1</strong> + <strong>Audio Adapter</strong> - The brain and audio interface</li>
            <li><strong>Adafruit MIDI FeatherWing</strong> - MIDI DIN I/O, although we only use the input</li>
            <li><strong>Adafruit NeoKey 1X4 switches</strong> - For all interactions with effects</li>
            <li><strong>CYT1100 Rotary Encoders with switches</strong> - For settings menu navigation and selection</li>
            <li><strong>MCP23017 I2C I/O expander</strong> - Not enough pins on the Teensy without it, also adds interrupts to the encoders</li>
            <li><strong>8 MB PSRAM Chip</strong> - Allows for use of much larger loop buffers</li>
            <li><strong>MicroSD Card</strong> - Non-volatile storage for loop presets</li>
            <li><strong>SSD1306 128x64 OLED Display</strong> - For settings menu navigation and effects feedback</li>
            <li><strong>Red LEDs</strong> - Visualizes preset slot selection and operations</li>
            <li><strong>Green LED</strong> - Visualizes MIDI clock</li>
            <li><strong>RGB LED</strong> - Visualizes feedback for effect states and loop capture</li>
          </ul>
        </section>

        <div className="my-10" aria-hidden="true" />

        {/* Bottom Home Link */}
        <Link to="/" className="text-accent hover-slant text-lg inline-block">
          &lt;- Home
        </Link>
      </div>
    </div>
  )
}


