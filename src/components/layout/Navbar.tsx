...
const navLinks = [
  { to: 'https://github.com/levon-m', label: 'GitHub', internal: false, icon: 'github' },
  { to: 'https://www.linkedin.com/in/levonmelkonyan/', label: 'LinkedIn', internal: false, icon: 'linkedin' },
  { to: '/resume', label: 'Resume', internal: true, icon: 'resume' },
]
            ...
            {navLinks.map((link) =>
              link.internal ? (
                <Link
                  ...
                  className={`px-4 py-2 text-sm font-medium rounded-md transition-all flex items-center gap-2 ${
                    isActive(link.to)
                      ...
                      : 'text-surface hover:italic hover:bg-cream/10'
                  }`}
                >
                  {getIcon(link.icon)}
                  {link.label}
                ...
                <a
                  ...
                  className="px-4 py-2 text-sm font-medium rounded-md text-surface hover:italic hover:bg-cream/10 transition-all flex items-center gap-2"
                >
                  {getIcon(link.icon)}
                  {link.label}
                </a>
              ...
              {navLinks.map((link) =>
                link.internal ? (
                  ...
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() => setMobileMenuOpen(false)}
                    className="flex items-center gap-2 px-4 py-2 text-base rounded-md text-surface hover:italic hover:bg-cream/10 transition-all"
                  >
                    {getIcon(link.icon)}
                    {link.label}
                  </a>
